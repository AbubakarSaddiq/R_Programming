...
```{r }
name<-c("A","b","u","b","a","k","a","r")
name
```


```{r }
concate = paste(name,sep="", collapse="")
concate
```
...
```{r }
even_num = matrix(seq(0,16,2),ncol=3,byrow=TRUE)
even_num
```
...
```{r }
var<-factor(c("Employed","unemployed","self-employed","Employed","unemployed","self-employed","Employed","unemployed","self-employed"))
var
```
...
```{r }
df<-data.frame(CustomerID=c("Alicia","Nancy","Fernando","Louis","Dane") , hasAccount =c(F,T,T,T,NA), totalBalance =c( NA,10000,100,2000,NA))
df
```
